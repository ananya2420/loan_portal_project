...
import { setLoanDetails, clearLoanDetails } from '../store/formSlice';
...
const LoanDetails = () => {
  ...
  const {
    ...
    mode: 'onChange',
    defaultValues: { amount: '', type: '', repaymentTerm: '', emiDate: '' },
  });
        ...
        {[...Array(totalSteps)].map((_, step) => {
          ...
          return (
            <div key={step} className="flex flex-col items-center col-span-1">
              ...
              <span className="mt-1 truncate" title={labels[step]}>
                {labels[step]}
              </span>
            </div>
          );
