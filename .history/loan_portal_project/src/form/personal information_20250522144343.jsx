...
const PersonalInformation = ({ setUserData, userData }) => {
  ...
  const defaultFormValues = shouldReset
    ? { name: '', dob: '', phone: '', email: '' }
    : userData || { name: '', dob: '', phone: '', email: '' };
  ...
  const onSubmit = (data) => {
    ...
    navigate('/apply/employee-details'); // no reset flag needed here
  };
  ...
  const handleBack = () => {
    navigate('/apply'); // Back to Apply (you can set a reset if needed)
  };
          ...
          {[...Array(8)].map((_, i) => {
            ...
            return (
              <div key={step} className="flex flex-col items-center col-span-1">
                ...
                <span className="mt-1 truncate whitespace-pre-line">{labels[i]}</span>
              </div>
            );
          })}
        </div>
