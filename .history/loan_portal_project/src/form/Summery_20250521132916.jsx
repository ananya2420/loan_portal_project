  ...
  return (
    <div className={`min-h-screen p-6 transition-all duration-300 ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`}>
      ...
      <div className="max-w-4xl mx-auto p-6 rounded-xl shadow-lg space-y-6 bg-white">
        ...
        {['personalInfo', 'employeeDetails', 'loanDetails', 'documentUpdates'].map((sectionKey) => {
          const sectionTitles = {
          ...
          return (
            <section
              key={sectionKey}
              className="p-5 rounded-lg border bg-gray-50 border-gray-200 group relative"
            >
              <h3 className="text-xl font-semibold mb-3">{sectionTitles[sectionKey]}</h3>

              {editingSection === sectionKey ? (
                <>
                  {fieldsMap[sectionKey].map((field) => (
                    <div key={field} className="mb-3">
                      <label className="block font-semibold mb-1 capitalize">{field}</label>
                      {field === 'isUpdated' ? (
                        <select
                          {...register(`${sectionKey}.${field}`, {
                        ...
                        <input
                          ...
                          type={
                            ['income', 'experience', 'amount', 'repaymentTerm'].includes(field)
                              ? 'number'
                              : 'text'
                          }
                          className="w-full border rounded px-4 py-2"
                      ...
                      {errors?.[sectionKey]?.[field] && (
                        <p className="text-red-500 text-sm mt-1">
                          {errors[sectionKey][field].message}
                        </p>
                      )}
                    </div>
                  ))}
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {fieldsMap[sectionKey].map((field, j) => (
                      <div key={j} className="text-sm">
                        <strong>{field.charAt(0).toUpperCase() + field.slice(1)}:</strong>{' '}
                        {renderMultipleValues(sectionData?.[field])}
                      </div>
                    ))}
                  </div>
                  ...
                  <button
                    ...
                    className="absolute top-5 right-5 px-3 py-1 text-xs bg-yellow-500 text-white rounded opacity-0 group-hover:opacity-100 transition"
                  >
                    Edit
                  </button>
                </>
