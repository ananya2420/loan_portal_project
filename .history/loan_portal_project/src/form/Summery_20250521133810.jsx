...
const Summary = () => {
  ...
  // Helper to render multiple values as list, split by '/' or ','
  const renderMultipleValues = (value) => {
    if (!value) return 'N/A';
    // Split by slash or comma, trim each item
          ...
          <>
            {[
              ...
              },
              {
                ...
                content: [
                  { label: 'Status', value: "full-ti" },
                  { label: 'Company', value: employeeDetails?.company },
                  {
                    label: 'Monthly Income',
                    value: employeeDetails?.income
                ...
                content: [
                  {
                    label: 'ID Updated',
                    value:
                      documentUpdates?.isUpdated === 'false' ? 'No' : 'Yes',
                  },
                ],
              },
            ].map((section, i) => (
              <section
                key={i}
                className="p-5 rounded-lg border bg-gray-50 border-gray-200"
              >
                  ...
                  {section.content.map((item, j) => (
                    <div key={j} className="text-sm">
                      ...
                      {item.label.includes('Income') || item.label.includes('Term') || item.label.includes('Amount') || item.label.includes('Experience') ? (
                        // For amounts and experience, show as normal string (no splitting)
