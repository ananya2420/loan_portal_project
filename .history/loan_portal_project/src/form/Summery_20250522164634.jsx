            ...
            {['name', 'dob', 'phone', 'email'].map((field) => (
              <div key={field}>
                ...
                >
                  ...
                  {field === 'name' && ['gourab', 'bob'].map((v) => <option key={v} value={v}>{v}</option>)}
                  {field === 'dob' && ['21.5.2025'].map((v) => <option key={v} value={v}>{v}</option>)}
                  {field === 'phone' && ['01700', '01654'].map((v) => <option key={v} value={v}>{v}</option>)}
                  {field === 'email' && ['aborty@gmail.com', 'a@gmail.com'].map((v) => <option key={v} value={v}>{v}</option>)}
                </select>
        ...
        return (
          <>
            ...
            {[{ name: 'company', options: ['Brain station', 'tech solution'] },
              ...
              { name: 'experience', options: ['5', '6'] }].map(({ name, options }) => (
              <div key={name}>
        ...
        return (
          <>
            ...
            {[{ name: 'amount', options: ['20000', '50000'] },
              ...
              { name: 'emiDate', options: ['21.5.2025', '22.5.2025'] }].map(({ name, options }) => (
              <div key={name}>
  ...
  return (
    <div className={`min-h-screen p-4 ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`}>
      ...
      <div className="flex justify-between max-w-md mx-auto mb-4 px-2">
        {['Apply', 'Personal Info', 'Employee Details', 'Loan Details', 'Document Updates', 'Summary', 'Review', 'Thank You']
          .map((label, index) => {
            ...
            {[
              {
                ...
                content: [
                  { label: 'Name', value: personalInfo?.name || 'Gourab' },
                  { label: 'Date of Birth', value: personalInfo?.dob || '21.05.2025' },
                  { label: 'Phone', value: personalInfo?.phone || '01700-675325' },
                  { label: 'Email', value: personalInfo?.email || 'aborty@gmail.com' },
                ],
              },
              {
                title: 'Employee Details',
                content: [
                  { label: 'Status', value: employeeDetails?.status || 'Full time' },
                  { label: 'Company', value: employeeDetails?.company || 'Brain Station' },
                  { label: 'Monthly Income', value: employeeDetails?.income || '10,000' },
                  { label: 'Experience', value: employeeDetails?.experience ? `${employeeDetails.experience} years` : '5 years' },
                  { label: 'Tax ID', value: employeeDetails?.taxId || 'tx555055' },
                ],
              },
              {
                title: 'Loan Details',
                content: [
                  { label: 'Amount', value: loanDetails?.amount || '20000' },
                  { label: 'Type', value: loanDetails?.type || 'Home Loan' },
                  { label: 'Term', value: loanDetails?.repaymentTerm ? `${loanDetails.repaymentTerm} months` : '6 months' },
                  { label: 'EMI Date', value: loanDetails?.emiDate || '21.5.25' },
                ],
              },
              {
