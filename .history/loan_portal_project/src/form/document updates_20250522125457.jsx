...
const DocumentUpdates = () => {
  ...
  // onChange handler to show preview
  const handleFileChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      setPreview(URL.createObjectURL(file));
      setValue('document', e.target.files); // update react-hook-form's value manually
    } else {
      setPreview(null);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Upload Photo label triggers hidden file input */}
        <div className="mb-6">
          <label
          ...
          <input
            ...
            accept="image/*"
            {...register('document')}
            onChange={handleFileChange}  // <-- add this handler here
            className="hidden"
          />
