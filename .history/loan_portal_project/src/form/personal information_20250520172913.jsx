...
const PersonalInfo = () => {
  ...
  // Autocomplete filtering for cities
  useEffect(() => {
    setFilteredCities(
      watchCity
        ? cities.filter((c) =>
  ...
  // Autocomplete filtering for states
  useEffect(() => {
    setFilteredStates(
      watchState
        ? states.filter((s) =>
  ...
  // Count how many valid fields are filled (no errors)
  const validFieldsCount = Object.keys(allFields).filter(
    (field) => allFields[field] && !errors[field]
  ).length;

  const onSubmit = (data) => {
    // You might want to dispatch this data to redux here
