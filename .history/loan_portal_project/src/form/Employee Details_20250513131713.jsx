...
import { useDispatch, useSelector } from 'react-redux'; // ⬅️ Added
import { setEmployeeDetails } from '../store/formSlice'; // ⬅️ Added
//import ProgressBar from '../components/progressbar';
import ProgressBar from '../components/ProgressBar';

const EmployeeDetails = () => {
  const dispatch = useDispatch(); // ⬅️ Added
  const navigate = useNavigate();

  const savedData = useSelector((state) => state.formData.employeeDetails); // ⬅️ Get persisted data
  const { register, handleSubmit, formState: { errors }, watch } = useForm({
    defaultValues: savedData || {} // Use persisted data as default
  });
  ...
  const onSubmit = (data) => {
    dispatch(setEmployeeDetails(data)); // ⬅️ Save to Redux store
