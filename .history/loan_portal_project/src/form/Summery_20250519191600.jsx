...
const Summary = () => {
  ...
  // Adjust this selector if your data is nested differently!
  const userData = useSelector((state) => state.formData || {});
  console.log('userData from Redux:', userData); // Debug log
  ...
  // Reset form when employeeDetails update
  useEffect(() => {
    reset(employeeDetails);
  }, [employeeDetails, reset]);

  // Redirect if personalInfo missing
  useEffect(() => {
    if (!personalInfo?.name) {
      alert('No user data available. Please complete the form steps before accessing the summary.');
      navigate('/apply/document-updates'); // Adjust to your start page
    }
  }, [personalInfo, navigate]);

  // Submit handler for employee details update
  const submitEmployeeDetails = (data) => {
    // TODO: Dispatch action to update Redux with new employeeDetails
    ...
    >
      ...
      >
        ...
        {isEditingEmployee ? (
          ...
          <>
            {/* Debug display to check data (remove in production) */}
            <section className="mb-6 p-4 bg-yellow-50 border border-yellow-300 rounded">
              <h3 className="text-lg font-semibold mb-2">DEBUG: Redux Data</h3>
              <pre className="text-xs overflow-auto max-h-48">{JSON.stringify(userData, null, 2)}</pre>
            </section>
