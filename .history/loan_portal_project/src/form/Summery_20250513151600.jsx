...
const Summery = ({ onEdit, onSubmit, mode }) => {
  ...
  const submitUpdatedInfo = (data) => {
    dispatch(setPersonalInfo(data)); // Save updated info to Redux
      ...
      {mode === 'edit' ? (
        <form onSubmit={handleSubmit(submitUpdatedInfo)} className="bg-gray-50 p-4 rounded-lg space-y-4">
          ...
          <div>
            ...
            <input type="tel" {...register('phone', { required: 'Phone number is required' })} className="w-full border rounded p-2" />
        ...
        <>
          ...
          <div className="flex justify-center gap-4 mt-6">
            ...
            <button onClick={handleFinalSubmit} className="px-5 py-2 bg-green-600 text-white rounded hover:bg-green-700">Submit</button>
          </div>
        </>
      )}
    </div>
