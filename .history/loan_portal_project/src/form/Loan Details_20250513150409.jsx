...
const LoanDetails = () => {
  ...
  const loanDetails = useSelector((state) => state.formData?.loanDetails || {}); // Ensure safe access to state.formData

  // Use React Hook Form to manage form state
  const { register, handleSubmit, formState: { errors } } = useForm({
    defaultValues: loanDetails, // Set default values for the form
  });
  ...
  useEffect(() => {
    if (!loanDetails) {
      console.error('Loan details are not available in Redux store.');
    }
  }, [loanDetails]);
