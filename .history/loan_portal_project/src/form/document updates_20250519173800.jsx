...
import { setDocumentUpdates } from '../store/formSlice';  // You need to create this action in your slice

const DocumentUpdates = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const theme = useSelector((state) => state.theme.theme);
  const savedData = useSelector((state) => state.formData?.documentUpdates); // load saved data if any
  ...
  const {
    ...
    mode: 'onChange',
    defaultValues: savedData || { amount: 50000, type: 'Home Loan' },
  ...
  const onSubmit = (data) => {
    dispatch(setDocumentUpdates(data));  // Save data to Redux store
    navigate('/apply/summary');          // Navigate to next step (adjust as needed)
  };
