...
const Summery = ({ userData, onEdit, onSubmit, mode, onNext, onBack }) => {
  ...
  // State to manage the visibility of the Review and Login buttons
  const [showReviewButtons, setShowReviewButtons] = useState(false);

  // Function to handle 'Next' button click
  const handleNext = () => {
    setShowReviewButtons(true); // Show the Review and Login buttons after clicking Next
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white dark:bg-gray-900 text-black dark:text-white rounded-lg shadow-md space-y-6">
      ...
      {mode === 'edit' ? (
        <form onSubmit={handleSubmit(onSubmit)} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg space-y-4">
        ...
        <>
          ...
          <div className="flex justify-between gap-4 mt-6">
            ...
            <div className="flex gap-4">
              ...
              {/* Handle Next Button */}
              <button onClick={handleNext} className="px-5 py-2 bg-green-600 text-white rounded hover:bg-green-700">Next</button>
            </div>
          </div>
        </>
      )}

      {/* Conditional Rendering for Review and Login Buttons */}
      {showReviewButtons && (
        <div className="flex justify-between gap-4 mt-6">
          <button onClick={handleReview} className="px-5 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Review</button>
          <button onClick={handleLogin} className="px-5 py-2 bg-red-500 text-white rounded hover:bg-red-600">Login</button>
        </div>
      )}
    </div>
  );
