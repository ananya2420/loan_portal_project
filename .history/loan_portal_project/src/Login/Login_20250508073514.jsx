...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (action === "login") {
      // Simulated login check - change logic as needed
      if (
        (emailInput === 'test@example.com' || userName === 'testuser') &&
        passwordInput === 'password123'
      ) {
        setUser({ email: emailInput, userName: userName || 'Test User' });
        setError('');
      } else {
        setError('Invalid username/email or password');
      }
    } else {
      // Register user locally (no backend)
      const newUser = {
