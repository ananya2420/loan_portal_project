...
import { setConfirmation } from '../redux/slices/formSlice'; // Import setConfirmation

const Review = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const theme = useSelector((state) => state.theme.theme); // Get the current theme from Redux state
  const confirmation = useSelector((state) => state.formData.confirmation); // Get confirmation value from Redux
  ...
  const {
    ...
    defaultValues: {
      confirmation: confirmation || false, // Set default value from Redux
    },
  });

  const onSubmit = (data) => {
    dispatch(setConfirmation(data.confirmation)); // Save confirmation to Redux
  ...
  const handleNext = () => {
    navigate('/next-step'); // Navigate to the next step page (ensure this route exists in App.js)
  };
