...
const Apply = () => {
  ...
  const onSubmit = async (data) => {
    if (step === 1) {
      ...
      const isValid = await trigger(['name', 'status']);
      if (isValid) {
        dispatch(setPersonalInfo(getValues()));
        setStep(2); // Move to personal-info summary step
      }
    } else if (step === 2) {
      // Move from personal-info summary to loan details (no validation here)
      setStep(3);
    } else if (step === 3) {
      // Validate loan details
      const isValid = await trigger(['amount', 'type']);
      if (isValid) {
