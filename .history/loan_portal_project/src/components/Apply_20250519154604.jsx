...
const Apply = () => {
  ...
  const onSubmit = async () => {
    ...
    if (step === 1) {
      isValid = await trigger(['name', 'status']);
      if (isValid) {
        const values = getValues();
        dispatch(setPersonalInfo({ name: values.name }));
        dispatch(setEmployeeDetails({ status: values.status }));
        setStep(2);
      }
    } else if (step === 2) {
