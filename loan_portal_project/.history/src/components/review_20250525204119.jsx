    ...
    >
      ...
      <div className="w-full max-w-md flex justify-end mb-6">
        <button
          ...
          className="text-sm px-3 py-1 bg-violet-600 text-white rounded hover:bg-violet-700 transition"
        >
          Toggle Theme
        </button>
      </div>
          ...
          <div key={step} className="flex flex-col items-center col-span-1">
            <span
              className={`w-7 h-7 flex items-center justify-center rounded-full font-bold ${
                step === currentStep ? stepColors[index] : 'bg-gray-300 text-gray-600'
              }`}
            >
              {step}
            </span>
      ...
      <div className="w-full max-w-md mx-auto">
        <div className="relative pt-1 mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="inline-block px-2 py-1 text-xs font-semibold text-purple-800 uppercase bg-purple-200 rounded-full">
              Step 7 of 8
            </span>
            <span className="text-xs font-semibold text-purple-800">70%</span>
          </div>
      ...
      >
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 text-left">
          <div>
            <label className="inline-flex items-center">
              <input
                type="checkbox"
                className="w-5 h-5 text-amber-500 form-checkbox"
        ...
        <div className="flex justify-center gap-4 mt-6">
          <button
            onClick={handleBack}
            className="px-5 py-2 text-white bg-rose-500 rounded hover:bg-rose-600"
          >
            Back
          </button>
        </div>
