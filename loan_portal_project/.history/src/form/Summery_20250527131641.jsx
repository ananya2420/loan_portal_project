  ...
  return (
    <div className={`min-h-screen p-6 ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`}>
      ...
      <div className="max-w-3xl mx-auto mb-4">
        <div className="mb-6 grid grid-cols-8 gap-x-3 gap-y-2 text-xs font-semibold text-center" flex justify-between text-xs font-semibold text-gray-500 dark:text-gray-400">
          {[
            'Apply',
            'Personal Info',
            'Employee Details',
            ...
            return (
              <div
                ...
                className={`flex flex-col items-center w-full cursor-pointer ${isActive ? 'text-emerald-600 font-bold' : ''}`}
                style={{ minWidth: '70px' }}
              >
                <div
                  className={`rounded-full w-6 h-6 flex items-center justify-center mb-1 border ${
                    isActive ? 'border-emerald-600 bg-emerald-100' : 'border-gray-300 bg-gray-100'
                  }`}
                >
                  {stepNumber}
                </div>
      ...
      >
        ...
        {isEditing ? (
          ...
          >
            <h2 className="text-center font-semibold text-xl">{stepTitles[editStep]}</h2>
            {renderStepForm()}
          ...
          <>
                ...
                content: [
                  {
                    ...
                    value:
                      documentUpdates?.isUpdated === 'true' || documentUpdates?.isUpdated === true
                        ? 'Yes'
                        : documentUpdates?.isUpdated === 'false' || documentUpdates?.isUpdated === false
                        ? 'No'
                        : 'N/A',
                  },
                  {
                    label: 'Preview',
                    value: documentUpdates?.preview || null,
                  },
                ],
              },
            ].map((section, i) => (
            ...
            <div className="flex flex-wrap gap-4 justify-between mt-6">
              ...
              <button
                ...
                className="px-8 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700 transition"
              >
                Next
              </button>
            </div>
