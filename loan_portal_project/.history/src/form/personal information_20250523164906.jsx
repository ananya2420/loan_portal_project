...
const PersonalInformation = () => {
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center p-6 transition-all duration-300 ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-black'}`}>
      <div className="w-full max-w-md">
        ...
        <div className="mb-6 grid grid-cols-8 gap-2 text-xs font-semibold text-center">
          {[
            ...
            <div key={step} className="flex flex-col items-center col-span-1">
              <span className={`w-7 h-7 flex items-center justify-center rounded-full font-bold ${step === currentStep ? 'bg-blue-600 text-white' : 'bg-gray-300 text-purple-600'}`}>
                {step}
              </span>
              <span className="mt-1 truncate text-purple-600" title={typeof label === 'string' ? label : undefined}>
                {label}
              </span>
            </div>
          ))}
        ...
        <div className="relative pt-1 mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="inline-block px-2 py-1 text-xs font-semibold text-orange-700 uppercase bg-orange-200 rounded-full">
              Step 2 of 8
            </span>
            <span className="text-xs font-semibold text-orange-700">20%</span>
          </div>
          <div className="flex h-2 overflow-hidden text-xs bg-orange-200 rounded">
            <div style={{ width: '20%' }} className="flex flex-col justify-center text-center text-white transition-all duration-500 bg-green-500 shadow-none whitespace-nowrap" />
          </div>
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit(onSubmit)} autoComplete="off" className={`shadow-lg rounded-lg p-8 w-full space-y-4 transition-all duration-300 ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'}`}>
          <h2 className="mb-4 text-2xl font-bold text-center">Personal Information</h2>

          {/* Full Name */}
          <div className="relative" onMouseEnter={() => setHoverField('name')} onMouseLeave={() => setHoverField('')}>
          ...
          {/* Phone */}
          <div className="relative" onMouseEnter={() => setHoverField('phone')} onMouseLeave={() => setHoverField('')}>
            <input
          ...
          {/* Email */}
          <div className="relative" onMouseEnter={() => setHoverField('email')} onMouseLeave={() => setHoverField('')}>
            <input
              type="email"
              placeholder="Email"
          ...
          <div className="flex justify-between mt-4">
            <button type="button" onClick={handleBack} className="px-6 py-3 text-white bg-rose-500 rounded-lg hover:bg-rose-600 transition duration-300">
              Back
            </button>
            <button
              type="submit"
        ...
        <div className="mt-6 p-4 rounded-lg bg-gray-100 dark:bg-gray-700 text-sm">
          ...
          <p><strong>Name:</strong> {allFields.name}</p>
          <p><strong>DOB:</strong> {allFields.dob}</p>
