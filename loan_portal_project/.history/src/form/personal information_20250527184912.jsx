...
const PersonalInformation = () => {
  ...
  // Sync selectedDate and form fields with Redux personalInfo on mount or when it changes
  useEffect(() => {
    if (personalInfo) {
      if (personalInfo.dob) {
  ...
  // New: Save form data to Redux on every form change so data persists across navigation
  useEffect(() => {
    dispatch(setPersonalInfo(allFields));
  }, [allFields, dispatch]);

  const onSubmit = (data) => {
    // The data is already saved on every change,
    // but dispatch again here to ensure it's saved before navigating
