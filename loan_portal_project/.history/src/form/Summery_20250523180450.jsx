...
const Summary = () => {
  ...
  // Define colors for each step number and label
  const stepColors = [
      ...
      <div className="max-w-3xl mx-auto mb-4">
        <div className="flex justify-between text-xs font-semibold text-gray-500 dark:text-gray-400">
          {[
            ...
            const isActive = stepNumber === 6; // Step 6 is "Summary" and active
            const colors = stepColors[index];
            return (
              <div
                key={stepLabel}
                className={`flex flex-col items-center w-full ${
                  isActive ? colors.text + ' font-bold' : colors.text + ' opacity-70'
                }`}
                style={{ minWidth: '70px' }}
              >
                <div
                  className={`rounded-full w-6 h-6 flex items-center justify-center mb-1 border ${
                    isActive
                      ? `${colors.numberBorder} ${colors.numberBg}`
          ...
          <>
            {[
              ...
              <section
                ...
                className={`p-5 rounded-lg border ${
                  theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-300'
                } space-y-3 shadow-sm transition-colors duration-300`}
              >
                <h3 className="text-xl font-semibold text-pink-600">{section.title}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-700 dark:text-gray-300">
                  {section.content.map((item, j) => (
                    <div key={j} className="break-words">
                      <strong className="text-pink-600">{item.label}:</strong>{' '}
                      {item.label === 'Preview' && item.value ? (
                        <img
                          src={item.value}
                          alt="Document Preview"
                          className="mt-2 max-h-48 rounded border border-indigo-300 shadow-sm"
                        />
                      ) : (
                        item.value
                      )}
            ...
            <div className="flex flex-wrap gap-3 justify-between mt-6">
              <button
                ...
                className="px-5 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded text-sm transition"
              >
                Back
              </button>
              ...
              <button
                ...
                className="px-5 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded text-sm transition"
              >
                View Full Summary
              </button>
              ...
              <button
                ...
                className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white rounded text-sm transition"
              >
                Continue
              </button>
              ...
              <button
                ...
                className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-sm transition"
              >
                Edit Info
              </button>
            </div>
