...
const PersonalInformation = () => {
  ...
  useEffect(() => {
    if (personalInfo && Object.keys(personalInfo).length && !initialized.current) {
  ...
  // Save current form data to Redux
  const saveFormData = () => {
    const currentData = {
      name: watch('name'),
      dob: watch('dob'),
  ...
  const onSubmit = (data) => {
    dispatch(setPersonalInfo(data)); // Save data to Redux
    navigate('/apply/employee-details'); // Next step
  };

  const handleBack = () => {
    saveFormData(); // Save data to Redux before navigating back
    navigate('/apply'); // Previous step
  };
